...
def show_image_iterm(data):
    from imgcat import imgcat
    from PIL import Image
    import io
    import base64
    dec = base64.b64decode(data['image/png'])
    image = Image.open(io.BytesIO(dec))
    # print all the fields (except image/png)
    print()
    imgcat(image)
    print()

def show_image_kitty(data):
    # uses a tempfile 
    # (but I'm rolling my own instead of the inbuilt jupyter tempfile)
    kittypath = '/opt/homebrew/bin/kitty'
    import subprocess
    import tempfile
    from PIL import Image
    import io
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix='.png') as fp:
        ...
        fp.write(dec)
        fp.seek(0)
        subprocess.run([kittypath, "icat", "--transfer-mode", "file", fp.name])
    print()


c.ZMQTerminalInteractiveShell.callable_image_handler = show_image_kitty
...
#  Default: []
# c.ZMQTerminalInteractiveShell.tempfile_image_handler = [kittypath,'+kitten','icat','--tmux','--transfer-mode','file','{file}']
# c.ZMQTerminalInteractiveShell.tempfile_image_handler = ['icatx','{file}']
